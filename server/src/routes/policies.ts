...
const parsePolicyRecommendation = (content: string): PolicyRecommendation => {
  ...
  try {
    ...
    const rawNotes = parsed.notes as unknown;
...
const buildUserPrompt = (body: PolicyRecommendationRequestBody): string => {
  ...
  lines.push(`현재 가용재고: ${product.onHand ?? '미상'} EA`);
  if (product.risk) {
    lines.push(`재고 리스크: ${product.risk}`);
  }
  if (typeof product.expiryDays === 'number') {
    lines.push(`유통기한 잔여일: ${product.expiryDays}일`);
  }
  lines.push(
    `현 정책: z=${policy.z ?? '미상'}, L=${policy.L ?? '미상'}일, R=${policy.R ?? '미상'}일, MOQ=${policy.moq ?? '미상'}, Pack=${policy.pack ?? '미상'}, CasePack=${policy.casePack ?? '미상'}, LT변동=${policy.includeLTVar ? '포함' : '미포함'}, sigmaL=${policy.sigmaL ?? '미상'}`,
  );
